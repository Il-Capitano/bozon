...
struct range_base_collect
{
	...
	constexpr auto collect(void) const noexcept;
...
public:
	...
	constexpr self_t &operator ++ (void)
	{
		++this->_it;
		return *this;
	}
...
template<typename T>
constexpr auto iota(T begin, T end) noexcept
{
	return iota_range{
		std::move(begin),
		std::move(end)
...
template<typename Range>
template<template<typename ...Ts> typename Vec>
constexpr auto range_base_collect<Range>::collect(void) const noexcept
{
