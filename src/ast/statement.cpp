...
function_body *function_body::add_specialized_body(std::unique_ptr<function_body> body)
{
	...
	if (it == this->generic_specializations.end())
	{
		...
		if (func_body->is_intrinsic())
		{
			...
			static_assert(_builtin_last - _builtin_first == 120);
			default:
				break;
			}
		}
...
((void)([]() { static_assert(kind == pos); }), create_builtin_function(kind, symbol_name, __VA_ARGS__))
	bz::vector result = {
		...
		add_builtin(34, function_body::memcpy,  "llvm.memcpy.p0i8.p0i8.i64",  void_type, void_ptr_type, void_const_ptr_type, uint64_type),
		add_builtin(35, function_body::memmove, "llvm.memmove.p0i8.p0i8.i64", void_type, void_ptr_type, void_const_ptr_type, uint64_type),
		add_builtin(36, function_body::memset,  "llvm.memset.p0i8.i64", void_type, void_ptr_type, uint8_type, uint64_type),

		add_builtin(37, function_body::exp_f32,   "llvm.exp.f32",   float32_type, float32_type),
		add_builtin(38, function_body::exp_f64,   "llvm.exp.f64",   float64_type, float64_type),
		add_builtin(39, function_body::exp2_f32,  "llvm.exp2.f32",  float32_type, float32_type),
		add_builtin(40, function_body::exp2_f64,  "llvm.exp2.f64",  float64_type, float64_type),
		add_builtin(41, function_body::expm1_f32, "expm1f",         float32_type, float32_type),
		add_builtin(42, function_body::expm1_f64, "expm1",          float64_type, float64_type),
		add_builtin(43, function_body::log_f32,   "llvm.log.f32",   float32_type, float32_type),
		add_builtin(44, function_body::log_f64,   "llvm.log.f64",   float64_type, float64_type),
		add_builtin(45, function_body::log10_f32, "llvm.log10.f32", float32_type, float32_type),
		add_builtin(46, function_body::log10_f64, "llvm.log10.f64", float64_type, float64_type),
		add_builtin(47, function_body::log2_f32,  "llvm.log2.f32",  float32_type, float32_type),
		add_builtin(48, function_body::log2_f64,  "llvm.log2.f64",  float64_type, float64_type),
		add_builtin(49, function_body::log1p_f32, "log1pf",         float32_type, float32_type),
		add_builtin(50, function_body::log1p_f64, "log1p",          float64_type, float64_type),

		add_builtin(51, function_body::sqrt_f32,  "llvm.sqrt.f32", float32_type, float32_type),
		add_builtin(52, function_body::sqrt_f64,  "llvm.sqrt.f64", float64_type, float64_type),
		add_builtin(53, function_body::pow_f32,   "llvm.pow.f32",  float32_type, float32_type, float32_type),
		add_builtin(54, function_body::pow_f64,   "llvm.pow.f64",  float64_type, float64_type, float64_type),
		add_builtin(55, function_body::cbrt_f32,  "cbrtf",         float32_type, float32_type),
		add_builtin(56, function_body::cbrt_f64,  "cbrt",          float64_type, float64_type),
		add_builtin(57, function_body::hypot_f32, "hypotf",        float32_type, float32_type, float32_type),
		add_builtin(58, function_body::hypot_f64, "hypot",         float64_type, float64_type, float64_type),

		add_builtin(59, function_body::sin_f32,   "llvm.sin.f32", float32_type, float32_type),
		add_builtin(60, function_body::sin_f64,   "llvm.sin.f64", float64_type, float64_type),
		add_builtin(61, function_body::cos_f32,   "llvm.cos.f32", float32_type, float32_type),
		add_builtin(62, function_body::cos_f64,   "llvm.cos.f64", float64_type, float64_type),
		add_builtin(63, function_body::tan_f32,   "tanf",         float32_type, float32_type),
		add_builtin(64, function_body::tan_f64,   "tan",          float64_type, float64_type),
		add_builtin(65, function_body::asin_f32,  "asinf",        float32_type, float32_type),
		add_builtin(66, function_body::asin_f64,  "asin",         float64_type, float64_type),
		add_builtin(67, function_body::acos_f32,  "acosf",        float32_type, float32_type),
		add_builtin(68, function_body::acos_f64,  "acos",         float64_type, float64_type),
		add_builtin(69, function_body::atan_f32,  "atanf",        float32_type, float32_type),
		add_builtin(70, function_body::atan_f64,  "atan",         float64_type, float64_type),
		add_builtin(71, function_body::atan2_f32, "atan2f",       float32_type, float32_type, float32_type),
		add_builtin(72, function_body::atan2_f64, "atan2",        float64_type, float64_type, float64_type),

		add_builtin(73, function_body::sinh_f32,  "sinhf",  float32_type, float32_type),
		add_builtin(74, function_body::sinh_f64,  "sinh",   float64_type, float64_type),
		add_builtin(75, function_body::cosh_f32,  "coshf",  float32_type, float32_type),
		add_builtin(76, function_body::cosh_f64,  "cosh",   float64_type, float64_type),
		add_builtin(77, function_body::tanh_f32,  "tanhf",  float32_type, float32_type),
		add_builtin(78, function_body::tanh_f64,  "tanh",   float64_type, float64_type),
		add_builtin(79, function_body::asinh_f32, "asinhf", float32_type, float32_type),
		add_builtin(80, function_body::asinh_f64, "asinh",  float64_type, float64_type),
		add_builtin(81, function_body::acosh_f32, "acoshf", float32_type, float32_type),
		add_builtin(82, function_body::acosh_f64, "acosh",  float64_type, float64_type),
		add_builtin(83, function_body::atanh_f32, "atanhf", float32_type, float32_type),
		add_builtin(84, function_body::atanh_f64, "atanh",  float64_type, float64_type),

		add_builtin(85, function_body::erf_f32,    "erff",    float32_type, float32_type),
		add_builtin(86, function_body::erf_f64,    "erf",     float64_type, float64_type),
		add_builtin(87, function_body::erfc_f32,   "erfcf",   float32_type, float32_type),
		add_builtin(88, function_body::erfc_f64,   "erfc",    float64_type, float64_type),
		add_builtin(89, function_body::tgamma_f32, "tgammaf", float32_type, float32_type),
		add_builtin(90, function_body::tgamma_f64, "tgamma",  float64_type, float64_type),
		add_builtin(91, function_body::lgamma_f32, "lgammaf", float32_type, float32_type),
		add_builtin(92, function_body::lgamma_f64, "lgamma",  float64_type, float64_type),

		add_builtin( 93, function_body::bitreverse_u8,  "llvm.bitreverse.i8",  uint8_type,  uint8_type),
		add_builtin( 94, function_body::bitreverse_u16, "llvm.bitreverse.i16", uint16_type, uint16_type),
		add_builtin( 95, function_body::bitreverse_u32, "llvm.bitreverse.i32", uint32_type, uint32_type),
		add_builtin( 96, function_body::bitreverse_u64, "llvm.bitreverse.i64", uint64_type, uint64_type),
		add_builtin( 97, function_body::popcount_u8,    "llvm.ctpop.i8",  uint8_type,  uint8_type),
		add_builtin( 98, function_body::popcount_u16,   "llvm.ctpop.i16", uint16_type, uint16_type),
		add_builtin( 99, function_body::popcount_u32,   "llvm.ctpop.i32", uint32_type, uint32_type),
		add_builtin(100, function_body::popcount_u64,   "llvm.ctpop.i64", uint64_type, uint64_type),
		add_builtin(101, function_body::byteswap_u16,   "llvm.bswap.i16", uint16_type, uint16_type),
		add_builtin(102, function_body::byteswap_u32,   "llvm.bswap.i32", uint32_type, uint32_type),
		add_builtin(103, function_body::byteswap_u64,   "llvm.bswap.i64", uint64_type, uint64_type),

		add_builtin(104, function_body::clz_u8,   "llvm.ctlz.i8",  uint8_type,  uint8_type),
		add_builtin(105, function_body::clz_u16,  "llvm.ctlz.i16", uint16_type, uint16_type),
		add_builtin(106, function_body::clz_u32,  "llvm.ctlz.i32", uint32_type, uint32_type),
		add_builtin(107, function_body::clz_u64,  "llvm.ctlz.i64", uint64_type, uint64_type),
		add_builtin(108, function_body::ctz_u8,   "llvm.cttz.i8",  uint8_type,  uint8_type),
		add_builtin(109, function_body::ctz_u16,  "llvm.cttz.i16", uint16_type, uint16_type),
		add_builtin(110, function_body::ctz_u32,  "llvm.cttz.i32", uint32_type, uint32_type),
		add_builtin(111, function_body::ctz_u64,  "llvm.cttz.i64", uint64_type, uint64_type),
		add_builtin(112, function_body::fshl_u8,  "llvm.fshl.i8",  uint8_type,  uint8_type,  uint8_type,  uint8_type),
		add_builtin(113, function_body::fshl_u16, "llvm.fshl.i16", uint16_type, uint16_type, uint16_type, uint16_type),
		add_builtin(114, function_body::fshl_u32, "llvm.fshl.i32", uint32_type, uint32_type, uint32_type, uint32_type),
		add_builtin(115, function_body::fshl_u64, "llvm.fshl.i64", uint64_type, uint64_type, uint64_type, uint64_type),
		add_builtin(116, function_body::fshr_u8,  "llvm.fshr.i8",  uint8_type,  uint8_type,  uint8_type,  uint8_type),
		add_builtin(117, function_body::fshr_u16, "llvm.fshr.i16", uint16_type, uint16_type, uint16_type, uint16_type),
		add_builtin(118, function_body::fshr_u32, "llvm.fshr.i32", uint32_type, uint32_type, uint32_type, uint32_type),
		add_builtin(119, function_body::fshr_u64, "llvm.fshr.i64", uint64_type, uint64_type, uint64_type, uint64_type),
	};
#undef add_builtin
