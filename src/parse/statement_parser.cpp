...
)
{
	...
	auto const arg_tokens = get_expression_tokens_without_error<
	...
	{
		return ast::make_stmt_static_assert(static_assert_pos, arg_tokens);
	}
	else
	{
		auto result = ast::make_stmt_static_assert(static_assert_pos, arg_tokens);
		resolve::resolve_statement(result, context);
		return result;
	}
}
...
)
{
	...
	if (condition.not_null() && !context.in_generic_function())
	{
...
)
{
	...
	context.assert_token(stream, lex::token::semi_colon);
	auto result = ast::make_stmt_return(return_pos, std::move(expr));
	resolve::resolve_statement(result, context);
	return result;
}
